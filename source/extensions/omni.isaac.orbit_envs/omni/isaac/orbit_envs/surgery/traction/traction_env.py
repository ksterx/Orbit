...
from .traction_cfg import RandomizationCfg, TractionEnvCfg


class TractionEnv(IsaacEnv):
    """Environment for reaching to desired pose for a single-arm manipulator."""

    def __init__(self, cfg: TractionEnvCfg = None, headless: bool = False):
        # copy configuration
        self.cfg = cfg
...
class ReachObservationManager(ObservationManager):
    ...
    def arm_dof_pos_normalized(self, env: TractionEnv):
...
class ReachRewardManager(RewardManager):
    ...
    def tracking_robot_position_l2(self, env: TractionEnv):
    ...
    def tracking_robot_position_exp(self, env: TractionEnv, sigma: float):
